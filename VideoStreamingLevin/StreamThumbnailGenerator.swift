...
typealias thumbnailCallBack = (_ image: UIImage) -> Void

public class StreamThumbnailGenerator: NSObject {
    ...
    var callback: thumbnailCallBack?
    ...
    public func replaceStreamUrl(newUrl url: URL) {
     ...
     func captureImage(at position: Double, completion: @escaping thumbnailCallBack) {
        guard !loading else {
        ...
        if let buffer = videoOutput.copyPixelBuffer(forItemTime: currentTime, itemTimeForDisplay: nil) {
            ...
            if let videoImage = CIContext().createCGImage(ciImage, from: imgRect) {
                ...
                if !queue.isEmpty {
                    ...
                    captureImage(at: position, completion: callBack)
                    }
                }
            }
        }
